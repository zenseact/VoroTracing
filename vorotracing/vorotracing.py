...
class VoroTracing(torch.nn.Module):
    ...
    def _prune_optimizer(self, mask):
        """Prune the optimizer by a mask. This is needed to keep the optimizer in a valid state."""
        assert self.optimizer is not None, (
            "Optimizer must be assigned with set_optimizer() first"
        )
    ...
    def _cat_tensors_to_optimizer(self, new_params):
        assert self.optimizer is not None, (
            "Optimizer must be assigned with set_optimizer() first"
        )
    ...
    def _densification_postfix(self, new_params):
        optimizable_tensors = self._cat_tensors_to_optimizer(new_params)
        self.primal_points = optimizable_tensors["primal_points"]
        self.att_diffuse = optimizable_tensors["att_diffuse"]
        self.att_specular = optimizable_tensors["att_specular"]
        self.density = optimizable_tensors["density"]

    def update_contribution_ema(
        self, contribution: torch.Tensor | None, decay: float = 0.99
    ) -> None:
        if contribution is None:
            return

        contribution = contribution.detach().float()
        if (
            not hasattr(self, "contribution_ema")
            or self.contribution_ema.shape != contribution.shape
        ):
            self.contribution_ema = torch.full_like(contribution, 1e-5)

        visible = contribution > 0
        self.contribution_ema[visible] = (
            decay * self.contribution_ema[visible]
            + (1.0 - decay) * contribution[visible]
        )
    ...
    def prune_and_densify(
        ...
        with torch.no_grad():
            ...
            if (
                hasattr(self, "contribution_ema")
                and self.contribution_ema.shape == point_contribution.shape
            ):
                point_contribution = torch.maximum(
                    point_contribution.float(), self.contribution_ema
                )
            ...
            ################### Farthest neighbor ###################
            farthest_neighbor, cell_radius = ops.farthest_neighbor(
                points,
                point_adjacency,
                point_adjacency_offsets,
            )
            ...
            invalid_mask = (farthest_neighbor == UINT32_MAX) | (
                farthest_neighbor >= num_curr_points
            )
            if invalid_mask.any():
                # For points with invalid neighbors, use the point itself (no perturbation)
                farthest_neighbor = torch.where(
                    invalid_mask,
                    torch.arange(
                        num_curr_points,
                        device=farthest_neighbor.device,
                        dtype=farthest_neighbor.dtype,
                    ),
                    farthest_neighbor,
                )
            ...
            new_params = {
                ...
                "att_diffuse": self.att_diffuse[sampled_inds],
                "att_specular": self.att_specular[sampled_inds],
            ...
            self._densification_postfix(new_params)
            if (
                hasattr(self, "contribution_ema")
                and self.contribution_ema.shape[0] == num_curr_points
            ):
                self.contribution_ema = torch.cat(
                    [self.contribution_ema, self.contribution_ema[sampled_inds]],
                    dim=0,
                )
            self._prune_points(prune_mask)
            if (
                hasattr(self, "contribution_ema")
                and self.contribution_ema.shape[0] == prune_mask.shape[0]
            ):
                self.contribution_ema = self.contribution_ema[~prune_mask]
            self.require_full_triangulation = True
            return prune_mask.sum().item(), sampled_points.shape[0]

    def collect_error_map(self, dataset, white_bkg=True, downsample=2):
        origins = dataset.poses[:, :3, 3]

        start_points = self.get_starting_point(
            origins.to(self.device, non_blocking=True)
        )

        point_error_accum = torch.zeros_like(self.primal_points[..., 0:1])
        point_contribution_accum = torch.zeros_like(self.primal_points[..., 0:1])
        ...
        for i in range(len(dataset)):
            ...
            # Downsample
            ray_batch = ray_batch[::downsample, ::downsample, ...]
            rgb_batch = rgb_batch[::downsample, ::downsample, :]

            start_points_batch = start_points[i].cuda()

            rgba_output, _, contribution, _, errbox, _ = self.forward(
                ray_batch, start_points_batch, return_contribution=True
            )

            # (H, W, 5, 4) -> (H, W, 4)
            if rgba_output.ndim == 4:  # multiple rays per pixel, average over them
                rgba_output = rgba_output.mean(dim=2)
